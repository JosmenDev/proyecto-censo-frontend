...
import { clearParentesco, guardarParentesco } from '../../../redux/Person/ParentescoSlice';

const FormParentesco = () => {

    const dispatch = useDispatch();
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm({mode: "onChange"});

    const { parentesco, loading, error } = useSelector((state) => state.parentesco);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (parentesco) {
            setValue('nombre', parentesco.nombre);
        }
    }, [parentesco, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarParentesco(formData));
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !parentesco? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
