...
import { clearSeguroSalud, guardarSeguroSalud } from '../../../redux/Person/SeguroSaludSlice';

const FormSeguroSalud = () => {

    const dispatch = useDispatch();
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm({mode: "onChange"});

    const { seguroSalud, loading, error } = useSelector((state) => state.seguroSalud);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (seguroSalud) {
            setValue('nombre', seguroSalud.nombre);
        }
    }, [seguroSalud, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarSeguroSalud(formData));
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !seguroSalud? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
...
export default FormSeguroSalud