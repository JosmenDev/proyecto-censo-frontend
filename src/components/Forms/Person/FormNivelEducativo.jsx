...
import { clearNivelEducativo, guardarNivelEducativo } from '../../../redux/Person/NivelEducativoSlice';

const FormNivelEducativo = () => {

    const dispatch = useDispatch();
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm({mode: "onChange"});

    const { nivelEducativo, loading, error } = useSelector((state) => state.nivelEducativo);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (nivelEducativo) {
            setValue('nombre', nivelEducativo.nombre);
        }
    }, [nivelEducativo, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarNivelEducativo(formData));
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !nivelEducativo? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
...
export default FormNivelEducativo