...
import { clearAccionEmergencia, guardarAccionEmergencia } from '../../../redux/Person/AccionEmergenciaSlice';

const FormAccionEmergencia = () => {

    const dispatch = useDispatch();
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm({mode: "onChange"});

    const { accionEmergencia, loading, error } = useSelector((state) => state.accionEmergencia);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (accionEmergencia) {
            setValue('nombre', accionEmergencia.nombre);
        }
    }, [accionEmergencia, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarAccionEmergencia(formData));
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !accionEmergencia? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
...
export default FormAccionEmergencia