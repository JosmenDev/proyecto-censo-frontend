...
import { clearEnfermedad, guardarEnfermedad } from '../../../redux/Person/EnfermedadSlice';
...
const FormEnfermedad = () => {
    ...
    const { enfermedad, loading, error } = useSelector((state) => state.enfermedad);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (enfermedad) {
            setValue('nombre', enfermedad.nombre);
        }
    }, [enfermedad, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarEnfermedad(formData));
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !enfermedad? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
