...
import { clearAbastecimientoAgua, guardarAbastecimientoAgua } from '../../../redux/HoustingCharacteristics/AbastecimientoAguaSlice';

const FormAbastecimientoAgua = () => {

    const dispatch = useDispatch();
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm({mode: "onChange"});

    const { abastecimientoAgua, loading, error } = useSelector((state) => state.abastecimientoAgua);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (abastecimientoAgua) {
            setValue('nombre', abastecimientoAgua.nombre);
        }
    }, [abastecimientoAgua, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarAbastecimientoAgua(formData));
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !abastecimientoAgua? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
...
export default FormAbastecimientoAgua