...
import { clearCloracion, guardarCloracion } from '../../../redux/HoustingCharacteristics/CloracionSlice';
...
const FormCloracion = () => {
    ...
    const { cloracion, loading, error } = useSelector((state) => state.cloracion);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (cloracion) {
            setValue('nombre', cloracion.nombre);
        }
    }, [cloracion, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarCloracion(formData));
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !cloracion? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
