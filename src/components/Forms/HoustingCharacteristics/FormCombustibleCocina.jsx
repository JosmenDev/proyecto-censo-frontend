...
import { clearCombustibleCocina, guardarCombustibleCocina } from '../../../redux/HoustingCharacteristics/CombustibleCocinaSlice';

const FormCombustibleCocina = () => {

    const dispatch = useDispatch();
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm({mode: "onChange"});

    const { combustibleCocina, loading, error } = useSelector((state) => state.combustibleCocina);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (combustibleCocina) {
            setValue('nombre', combustibleCocina.nombre);
        }
    }, [combustibleCocina, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarCombustibleCocina(formData));
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !combustibleCocina? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
...
export default FormCombustibleCocina