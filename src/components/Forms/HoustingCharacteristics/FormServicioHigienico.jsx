...
import { clearServicioHigienico, guardarServicioHigienico } from '../../../redux/HoustingCharacteristics/ServicioHigienicoSlice';

const FormServicioHigienico = () => {

    const dispatch = useDispatch();
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm({mode: "onChange"});

    const { servicioHigienico, loading, error } = useSelector((state) => state.servicioHigienico);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (servicioHigienico) {
            setValue('nombre', servicioHigienico.nombre);
        }
    }, [servicioHigienico, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarServicioHigienico(formData));
        // setAlerta({});
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !servicioHigienico? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
...
export default FormServicioHigienico