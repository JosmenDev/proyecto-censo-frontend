...
import { clearDisposicionBasura, guardarDisposicionBasura } from '../../../redux/HoustingCharacteristics/DisposicionBasuraSlice';

const FormDisposicionBasura = () => {

    const dispatch = useDispatch();
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm({mode: "onChange"});

    const { disposicionBasura, loading, error } = useSelector((state) => state.disposicionBasura);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (disposicionBasura) {
            setValue('nombre', disposicionBasura.nombre);
        }
    }, [disposicionBasura, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarDisposicionBasura(formData));
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !disposicionBasura? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
...
export default FormDisposicionBasura