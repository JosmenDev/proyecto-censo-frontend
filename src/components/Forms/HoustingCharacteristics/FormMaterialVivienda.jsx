...
import { clearMaterialVivienda, guardarMaterialVivienda } from '../../../redux/HoustingCharacteristics/MaterialViviendaSlice';

const FormMaterialVivienda = () => {

    const dispatch = useDispatch();
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm({mode: "onChange"});

    const { materialVivienda, loading, error } = useSelector((state) => state.materialVivienda);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (materialVivienda) {
            setValue('nombre', materialVivienda.nombre);
        }
    }, [materialVivienda, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarMaterialVivienda(formData));
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !materialVivienda? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
...
export default FormMaterialVivienda