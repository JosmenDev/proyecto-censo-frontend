...
import { clearMedidaProteccion, guardarMedidaProteccion } from '../../../redux/HoustingCharacteristics/MedidaProteccionSlice';

const FormMedidaProteccion = () => {

    const dispatch = useDispatch();
    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm({mode: "onChange"});

    const { medidaProteccion, loading, error } = useSelector((state) => state.medidaProteccion);
    const [ alerta, setAlerta ] = useState({});

    useEffect( () => {
        if (medidaProteccion) {
            setValue('nombre', medidaProteccion.nombre);
        }
    }, [medidaProteccion, setValue]);

    const onSubmit = async formData => {
        dispatch(guardarMedidaProteccion(formData));
        reset();
    }
                ...
                <div className="p-6.5">
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xsm:flex-row">
                        <button 
                            ...
                            disabled={loading}
                        >
                            {loading ? "Procesando..." : !medidaProteccion? "Guardar" : "Actualizar" }
                        </button>
                        <button 
                            type="button"
                            onClick={() => {
...
export default FormMedidaProteccion